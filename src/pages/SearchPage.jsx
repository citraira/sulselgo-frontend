...
export default function SearchPage() {
  ...
  // 1. Mengubah isMobile menjadi state agar perubahan ukuran layar terdeteksi aktif
  ...
  const wilayahList = [
    ...
    "Luwu", "Luwu Timur", "Luwu Utara", "Maros", "Pangkep", "Pinrang",
    "Sidrap", "Sinjai", "Soppeng", "Takalar", "Tana Toraja", "Toraja Utara", "Wajo"
  ];

  // 2. Event Listener untuk memantau perubahan ukuran layar secara real-time
  useEffect(() => {
    const handleResize = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await API.get(`/destinasi/search?q=${keyword}`);
      const data = res.data;
      setResults(data);
    } catch (err) {
  ...
  const handleTrendingClick = async (term) => {
    ...
    try {
      const res = await API.get(`/destinasi/search?q=${encodeURIComponent(term)}`);
      const data = res.data;
      setResults(data);
      setShowFullResult(true);
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      {matchedKabupatenList.length > 1 && (
        <div style={{ width: "88%", margin: "50px auto" }}>
          <h2 style={{ marginBottom: "25px", fontSize: "30px", fontWeight: "800" }}>
      ...
      {showFullResult && results.length > 0 && (
        <div style={{ width: "88%", margin: "50px auto" }}>
          <h2 style={{ marginBottom: "30px", fontSize: "34px", fontWeight: "800" }}>
            Destinasi Ditemukan
          </h2>
      ...
      {/* ================= SEARCH NOT FOUND ================= */}
      {showFullResult && results.length === 0 && matchedKabupatenList.length === 0 && keyword.trim() !== "" && (
        <div style={{ width: "88%", margin: "50px auto", background: "#fff", borderRadius: "28px", padding: "60px 30px", textAlign: "center", boxShadow: "0 10px 30px rgba(0,0,0,0.08)" }}>
          <div style={{ fontSize: "70px", marginBottom: "20px" }}>🔍</div>
          <h2 style={{ fontSize: "32px", fontWeight: "800", marginBottom: "14px", color: "#222" }}>
            Destinasi Tidak Ditemukan
          </h2>
          <p style={{ fontSize: "17px", color: "#666", lineHeight: "1.8" }}>
            Tidak ada hasil untuk <span style={{ color: "#800000", fontWeight: "700" }}>"{keyword}"</span>.
            <br />
            Coba gunakan kata kunci lain seperti: pantai, makassar, air terjun, toraja, danau, dll.
          </p>
        </div>
      )}
    </div>
